...
var signIn = function signIn(event) {
  ...
  api.signIn(userForm).then(ui.signInSuccess).then(api.getAcctStatus)
  // .then(api.checkBalance)
  .catch(ui.signInFailure);
};

var balanceDelete = function balanceDelete() {
  event.preventDefault();
  api.balanceDelete().then(ui.onBalanceDeleteSuccess);
  // .catch(console.error())
};
var signUp = function signUp(event) {
  ...
  if (userForm.credentials.password !== userForm.credentials.password_confirmation) {
    ...
    api.signUp(userForm).then(ui.signUpSuccess);
    // .then(initNewAcct())
...
var changePW = function changePW(event) {
  event.preventDefault();
  var userForm = getFormFields(this);
  api.changePW(userForm).then(ui.changePWSuccess).then(reset()).catch(ui.changePWFailure);
  // .catch(console.error())
};
...
var initNewAcct = function initNewAcct() {
  api.initNewAcct().then(ui.initNewAcctSuccess);
};
var postTransaction = function postTransaction(event) {
  event.preventDefault();
  // $('#trans-submit').attr('disabled', 'disabled')
  var userForm = getFormFields(this);
  api.postTransaction(userForm).then(api.checkBalance).then(ui.onPostTransactionSuccess);
};

var checkBalance = function checkBalance() {
  event.preventDefault();
  api.checkBalance().then(ui.displayBalance());
};
...
var getAcctStatus = function getAcctStatus() {
  return $.ajax({
    url: config.apiOrigin + '/balances',
    method: 'GET',
    headers: {
    ...
    success: function success(data) {
      var length = data.balances.length;
      if (length === 0) {
        initNewAcct();
      }
...
var initNewAcctSuccess = function initNewAcctSuccess() {
...
var onBalanceDeleteSuccess = function onBalanceDeleteSuccess() {
...
var displayBalance = function displayBalance(data) {
...
var onPostTransactionSuccess = function onPostTransactionSuccess() {
  $('#post-trans-form')[0].reset();
...
module.exports = {
  ...
  onPostTransactionSuccess: onPostTransactionSuccess,
  onBalanceDeleteSuccess: onBalanceDeleteSuccess,
  initNewAcctSuccess: initNewAcctSuccess
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
